# Acervo Knowledge Model

## Status

**Version:** 0.1
**Status:** Conceptual model
**Scope:** Initial Acervo product

This document defines the conceptual knowledge model for Acervo. It describes the domain entities, their purpose, relationships, and the principles that guide the model.
...
Acervo builds a persistent, traceable understanding of a software system from sources, source-level events, evidence, facts, validation, and human context.

Sources provide information. A **Commit** is a source-level event, especially a Git commit. A **ChangeEvent** is a change identified by Acervo and can be derived from one or more source events. **Evidence** is concrete information obtained from sources. **Facts** are concrete assertions derived from evidence. **Knowledge** is accepted and consolidated understanding; a Fact does not become Knowledge automatically.
...
Represents a source from which Acervo obtains information.
...
Represents a source-level event, especially a Git commit. It records what was committed at the source level; it is not the same thing as the change Acervo identifies from that source information.
...
Represents a change identified by Acervo. A ChangeEvent may be derived from one or more source events, including Commits.
...
A ChangeEvent represents what Acervo identified as a change. It does not itself determine why it happened, whether it is correct, whether it violates a Rule, or whether a resulting Fact is true.
...
Represents documentation generated by Acervo from the Knowledge Model.
...
1. **Evidence before inference.** Acervo prefers assertions supported by reliable evidence over unsupported inference.
2. **Commit and ChangeEvent are distinct.** A Commit is a source-level event; a ChangeEvent is the change Acervo identifies from one or more source events.
...
This model supports the initial Acervo product flow:
